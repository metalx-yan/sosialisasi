...
<div class="container-fluid">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('kelurahan.update', $get->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-3">
                        ...
                        <select name="kecamatan_id" id="" class="form-control" required>
                            ...
                            @foreach ($kecamatan as $item)
                                <option value="{{ $item->id }}" {{ $item->id == $get->id ? 'selected' : '' }}>{{ $item->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <a href="{{ route('kelurahan.index') }}" class="btn btn-warning btn-sm">Back</a>
            </form>
        </div>
    </div>
</div>
