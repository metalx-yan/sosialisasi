...
<div class="container-fluid">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('request.update', $get->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="row">
                <div class="col-md-3">
                        ...
                        <select name="item_id" id="" class="form-control {{ $errors->has('item_id') ? 'is-invalid' : ''}}" required>
                            ...
                            @foreach (App\Item::all() as $item)
                                <option value="{{ $item->id }}" {{ old("item_id", $get->item_id) == $item->id ? "selected" : "" }}>{{ $item->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="purchase_id" id="" class="form-control {{ $errors->has('purchase_id') ? 'is-invalid' : ''}}" required>
                            ...
                            @foreach (App\Purchase::all() as $purchase)
                                <option value="{{ $purchase->id }}" {{ old("purchase_id", $get->item_id) == $item->id ? "selected" : "" }}>{{ $purchase->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="date" name="date" value="{{ $get->date }}" class="form-control {{ $errors->has('date') ? 'is-invalid' : ''}}" required>
                ...
                <div class="row">
                    <div class="col-md-3">
                        ...
                        <textarea name="description" id="" cols="10" rows="2" class="form-control {{ $errors->has('description') ? 'is-invalid' : ''}}">{{ $get->description }}</textarea>
